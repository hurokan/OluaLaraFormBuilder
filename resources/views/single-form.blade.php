<!doctypehtml><html lang="en"><head><meta charset="utf-8"><meta content="IE=edge"http-equiv="X-UA-Compatible"><title>Modernize server</title><meta content=""name="description"><meta content="width=device-width,initial-scale=1"name="viewport"><meta content="{{ csrf_token() }}"name="csrf-token"><meta content="all,follow"name="robots"><link crossorigin="anonymous"href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css"integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN"rel="stylesheet"></head><body><div class="container-fluid"><div class="container"id="alert-container">@if(session('success'))<div class="alert alert-success"role="alert">{{ session('success') }}</div>@endif @if(session('error'))<div class="alert alert-danger"role="alert">{{ session('error') }}</div>@endif</div><section class="row"><div class="col-md-12"><div class="card mb-4"><div class="card-header"><h3 class="card-title">Create Module</h3></div><div class="card-body"><form action="{{ route('single-form.submit') }}"id="createItemForm"method="POST">@csrf<div class="row mb-3"><div class="col-md-3"><label class="form-label required-label"for="tableName">{{__('level.module_name')}} <i class="fa fa-info-circle"title="{{__('message.module_name_tooltip')}}"></i></label> <input class="form-control text-lowercase"id="name"maxlength="30"name="name"required value="{{ old('name') }}"></div></div><div class="row mb-3 field_items"><div class="col-md-3"><label class="form-label required-label"for="field_name">{{__('level.field_name')}} <i class="fa fa-info-circle"title="{{__('message.field_name_tooltip')}}"></i></label> <input class="form-control text-lowercase field_name"id="field_name"maxlength="30"name="field_name[]"required></div><div class="col-md-3"><label class="form-label required-label"for="levelName">{{__('level.level_name')}} <i class="fa fa-info-circle"title="{{__('message.level_name_tooltip')}}"></i></label> <input class="form-control"id="levelName"maxlength="30"name="level_name[]"required></div><div class="col-md-3"><label class="form-label required-label"for="dataType">{{__('level.data_type')}} <i class="fa fa-info-circle"title="{{__('message.data_type_tooltip')}}"></i></label> <select class="form-select"id="dataType"name="data_type[]"required><option value=""disabled selected>Choose...</option>@foreach($dataTypes as $parent=>$types)<optgroup label="{{$parent}}">@foreach($types as $type=>$length)<option value="{{$type}}">{{$type}} - {{json_encode($length)}}</option>@endforeach</optgroup>@endforeach</select></div><div class="col-md-1"><label class="form-label"for="fieldSize">{{__('level.field_size')}} <i class="fa fa-info-circle"title="{{__('message.field_size_tooltip')}}"></i></label> <input class="form-control"id="fieldSize"maxlength="4"name="field_size[]"></div><div class="col-md-1"><label class="form-label"for="default">{{__('level.default')}} <i class="fa fa-info-circle"title="{{__('message.default_value_tooltip')}}"></i></label> <input class="form-control"id="default"maxlength="8"name="default[]"></div><div class="col-md-1 mb-3"><label class="form-label"for="nullable">{{__('level.nullable')}}</label> <select class="form-select"id="nullable"name="nullable[]"><option value="Y">Yes</option><option value="N">No</option></select></div><div class="col-md-3"><label class="form-label required-label"for="inputType">{{__('level.input_type')}} <i class="fa fa-info-circle"title="{{__('message.input_type_tooltip')}}"></i></label> <select class="form-select"id="inputType"name="input_type[]"required><option value=""disabled selected>Choose...</option>@foreach($htmlInputElements as $key=>$value)<option value="{{$value}}">{{$key}}</option>@endforeach</select></div><div class="col-md-6 validationRuleSection"id="validationRuleSection"><div class="border border-primary p-3 rounded"><h5 class="border-bottom pb-2">{{__('level.validation_rules')}} <i class="fa fa-info-circle"title="{{__('message.validation_rules_tooltip')}}"></i></h5><div class="row"><div class="col-md-4"><select class="form-select validation_rule"id="validation_rule"name="validation_rule[]"><option value=""disabled selected>Choose...</option>@foreach($validationRules as $key => $data)<option value="{{$key}}"name="{{$data['value']}}">{{$data['value']}}</option>@endforeach</select></div><div class="col-md-6 dynamicValidationField"id="dynamicValidationField"></div><div class="col-md-1 align-items-end d-flex"><button class="btn addValidationRuleBtn btn-outline-info"type="button"id="addValidationRuleBtn">+</button> <button class="btn btn-danger removeValidationRuleBtn"type="button"id="removeValidationRuleBtn">-</button></div></div><div class="validationRulesItem"id="validationRulesItem"></div></div></div><div class="col-md-1 align-items-end d-flex"><button class="btn btn-primary"type="button"id="addItemBtn">+</button></div></div><div id="itemRows"></div><div class="mt-1"><button class="btn btn-primary"type="submit">{{__('level.submit_btn')}}</button></div></form></div></div></div></section></div><script crossorigin="anonymous"integrity="sha384-C6RzsynM9kWDrMNeT87bh95OGNyZPhcTNXj1NW7RuBCsyN/o0jlpcV8Qyq46cDfL"src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script><script crossorigin="anonymous"integrity="sha256-/JqT3SQfawRcv/BIHPThkBvs0OEvtFFmqPF/lYI/Cxo="src="https://code.jquery.com/jquery-3.7.1.min.js"></script><script>$(document).ready(function () {
...
@foreach($htmlInputElements as $key=>$value)
        ...
        <div class="col-md-6 validationRuleSection" id="validationRuleSection">
                                    <div class="border border-primary rounded p-3">
                                        <h5 class="border-bottom pb-2">{{__('level.validation_rules')}}</h5>
                                        <div class="row">
                                            <div class="col-md-4">
